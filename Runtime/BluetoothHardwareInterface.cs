...
using Hulan.Pillo.SDK.Behaviours;
using Hulan.Pillo.SDK.Enums;
using Hulan.Pillo.SDK.Structures;
...
namespace Hulan.Pillo.SDK {
  ...
  public class BluetoothLEHardwareInterface {
    ...
    private static extern void _PilloLog (string message);

    [DllImport ("__Internal")]
    private static extern void _PilloInitialize (bool asCentral, bool asPeripheral);
    ...
    public static BluetoothDeviceScript Initialize (bool asCentral, bool asPeripheral, Action action, Action<string> errorAction) {
      ...
      if (Application.isEditor == true) {
        // If in the editor, we'll just call the action right away.
        bluetoothDeviceScript?.SendMessage ("OnBluetoothMessage", "Initialized");
      } else {
        _PilloInitialize (asCentral, asPeripheral);
      }
    ...
    public static void ScanForBeacons (string[] proximityUUIDs, Action<BeaconResponseData> actionBeaconResponse) {
