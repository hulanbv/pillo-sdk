...
namespace Hulan.Pillo.SDK.Behaviours {
  public class BluetoothDeviceScript : MonoBehaviour {
    ...
    public Action<BeaconResponseData> DiscoveredBeaconAction;
    ...
    public void OnBluetoothMessage (string message) {
      if (message != null) {
        ...
        if (message.Length >= deviceInitializedString.Length && message.Substring (0, deviceInitializedString.Length) == deviceInitializedString) {
          ...
          if (parts.Length >= 7) {
            var iBeaconData = new BeaconResponseData ();
            ...
            if (!int.TryParse (parts[4], out iBeaconData.RSSI))
              iBeaconData.RSSI = 0;
            int deviceProximity = 0;
            if (!int.TryParse (parts[6], out deviceProximity))
              deviceProximity = 0;
            iBeaconData.deviceProximity = (DeviceProximity)deviceProximity;
